...
async function createReview({ productId, userId, title, post, rating }: any) {
  try {
    const {
      rows: [review],
    } = await client.query(
...
async function deleteReviewsByUserId(userId: number) {
  try {
...
async function userReviewExists(userId: number, productId: number) {
  try {
    const { rows } = await client.query(
      `
        SELECT * FROM reviews
...
async function deleteReviewsByProductId(productId: number) {
  try {
    const { rows } = await client.query(
      `
        DELETE FROM reviews
