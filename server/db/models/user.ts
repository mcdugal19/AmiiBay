...
async function createUser({ username, password, email }: any) {
  try {
...
async function getUser({ username, password }: any) {
  try {
...
import { deleteReviewsByUserId } from "./reviews.js";
...
async function mapOverUserRows(rows: any[], id: number) {
...
async function getUserWithCart(id: number) {
  try {
    const { rows } = await client.query(
      `
      SELECT users.id,
