...
import { authRequired, adminRequired } from "./utils";
...
usersRouter.post("/login", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (user) {
      ...
      res.send({ message: `Welcome back, ${(user as any).username}!`, user });
    } else {
      next({
...
usersRouter.get("/logout", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
usersRouter.get("/me", authRequired, async (req: any, res: Response, next: NextFunction) => {
  try {
    res.send(req.user);
  } catch (error) {
    next(error);
...
usersRouter.patch("/me", authRequired, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id, username, password, email } = req.body;
    const _user = await User.getUserByUsername(username);
    const _userEmail = await User.getUserByEmail(email);
    const updateUser: { [key: string]: any } = { id: id };
    if (username) {
      if (_user) {
        throw {
          name: "UserExistsError",
...
usersRouter.get("/", adminRequired, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
usersRouter.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.deleteUser(Number(userId));
...
usersRouter.get("/admin/:userId", adminRequired, async (req: Request, res: Response, next: NextFunction) => {
  try {
